...
app.get('/api/market/price', async (req, res) => {
  try {
    ...
    res.json({ 
      ...
      history: data.history || [data.currentPrice],
      buyVolume: data.totalBuyVolume || 0,
      sellVolume: data.totalSellVolume || 0
    });
  } catch (e) {
...
app.post('/api/wallet/exchange', async (req, res) => {
  ...
  try {
    ...
    await Save.findOneAndUpdate(
      ...
      {
        $inc: {
          'data.pixr': pixrChange,
          'data.gram': gramChange,
        }
      }
    );
    ...
    res.json({
      ...
      newPrice: newPrice,
      commission: COMMISSION,
      pixrChange: pixrChange,
      gramChange: gramChange,
      newBalances: newBalances,
      history: market.history,
      buyVolume: market.totalBuyVolume,
      sellVolume: market.totalSellVolume
    });
