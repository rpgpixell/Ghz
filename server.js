/*
  ══════════════════════════════════════════════════════
  server.js — Backend для PIXEL RPG (упрощённый)
  - Полное сохранение (снапшот)
  - Хеширование пароля
  - Экранирование HTML
  ══════════════════════════════════════════════════════
*/
...
app.post('/api/ref/claim', async (req, res) => {
  ...
  try {
    ...
    await Save.findOneAndUpdate(
      ...
      { 
        $set: { refMilestones: newMilestones }, 
        $inc: { refClaimVer: 1, 'data.gold': gold }
      }
    );
...
// ── Конфиг админов с хешированием ──
const ADMIN_PASSWORD_HASH = process.env.ADMIN_PASSWORD_HASH || 
  hashPassword(process.env.ADMIN_PASSWORD || 'pixel2024');
  ...
  try {
    ...
    if (bot) {
      ...
      for (const user of users) {
        try {
          await bot.sendMessage(user.tgId, safeMessage);
          sent++;
        } catch (e) {}
      }
    }
